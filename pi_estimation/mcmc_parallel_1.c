...
double estimate_pi_mcmc(long long int N) {
    ...
    #pragma omp parallel for reduction(+:inside_circle_count) private(seed) num_threads(20)
...
int main(int argc, char *argv[]) {
    ...
    // Print the number of threads (commented out as per your request)
    // #pragma omp parallel
    // {
    //     #pragma omp single
    ...
    printf("20 %lli %.20f %.20f %f\n", N, pi_estimate, error_estimate, end_total_time - start_total_time);
